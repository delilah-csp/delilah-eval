...
void
experiment_read_raw_direct_noncpy(struct io_uring* ring, int fd)
{
  ...
  sz = file_read(prog, PROG_SIZE, "programs/file3.o");
  ...
  ft_write_ln(table, "File Size", "Write Prog", "Write State", "Exec Prog",
              "Read Result", "Total Time");

  for (int i = 0; i < 6; i++) {
    file->size = (sizes[i] * 10) & 0xFFFFF000; // the size denotes the number of strings
    sprintf(file->filename, "/dev/nvme0n1");
    ...
    add_units(filesize, (sizes[i] * 10) & 0xFFFFF000);
