...
void
experiment_read_file_direct(struct io_uring* ring, int fd)
{
  ...
  uint64_t sizes[] = { 10000, 100000, 1000000, 10000000 };
  struct delilah_file_t* file = (struct delilah_file_t*)src;

  sz = file_read(prog, PROG_SIZE, "programs/file2.o");
  ...
  for (int i = 0; i < 4; i++) {
    ...
    memcpy(first, dst, 9);
    memcpy(last, dst + ((sizes[i] * 10) & 0xFFFFF000) - 10, 9);
