...
int main() {
  ...
  sz = file_read(prog, PROG_SIZE, "programs/file_map.o");
  for (int i = 0; i < 6; i++) {
    file->size = sizes[i] * 10; // the size denotes the number of strings
    switch (i) {
    case 0:
      sprintf(file->filename, "1kb");
      break;
    case 1:
      sprintf(file->filename, "10kb");
      break;
    case 2:
      sprintf(file->filename, "100kb");
      break;
    case 3:
      sprintf(file->filename, "1mb");
      break;
    case 4:
      sprintf(file->filename, "10mb");
      break;
    case 5:
      sprintf(file->filename, "100mb");
      break;
    }
